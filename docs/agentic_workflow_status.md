...
| §9.2 bounded autonomous worker | **Built** 2026-09-24 (`worker/`): contract → isolated worktree branch → headless Claude or Codex rounds with schema-checked proposals → scope check → the controller's own gates → review bundle or failure report; ledger per run. **3 of 10** reviewed attempts, all merged; tracked cluster gates and diagnosis contracts built | [worker/README.md](../worker/README.md); pilot 1 (Claude, guard argument order): 1 round, $0.53; pilot 2 (Codex, `--parameters-file`): one stop with a report (a controller prompt fault), then 1 round; pilot 3 (Claude, **diagnosis** of the missing bandgap tempco, tracked gate): 1 round, $0.63, 2 licensed runs |
