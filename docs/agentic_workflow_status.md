...
| §8 baseline and ablation (A–D) | Condition A **measured** 2026-09-24; B in ordinary use to be re-measured from 2026-10-09; the B-versus-C frozen-task design is written; §9.2 is done, so the B runner and the frozen task set are next | [baseline](agentic_baseline.md): tsmc65, 24 sessions in 25 days, 0 tracked, 375 licensed launches, 5,197 cluster reads, 12.2 h of in-command sleep; 0 of 965 harvested attempts has a declared failure cause |
