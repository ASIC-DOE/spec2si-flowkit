...
_BIN_FILES = ("report.sh", "runjob", "progress.py", "jobrec.py", "license.py", "evidence.py")
...
class Transport:
    ...
    def __init__(self, host=DEFAULT_HOST, mode=None, timeout=DEFAULT_TIMEOUT,
                 runner=None, isolated_bundle=False):
        self.host = host
        self.mode = mode or self._default_mode()
        self.timeout = timeout
        self._runner = runner or self._subprocess_runner
        self.bin_rel = (JOBS_DIRNAME + "/bundles/" + bundle_manifest()
                        if isolated_bundle else BIN_REL)
    ...
    def ensure_bin(self, timeout=None):
        ...
        if self._ensured.get(self.host) == want:
            return Result(KNOWN, self.host, data={"bundle": want,
                                                  "action": "cached"})
        installer = _bundle_installer(bin_files(), want, self.bin_rel)
    ...
    def read(self, subcommand="probe", *args, **kw):
        ...
        launcher = _reader_launcher(subcommand, args, self.bin_rel + "/report.sh")
    ...
    def evidence(self, contract, timeout=None):
        ...
        return self.run_sh('exec python3 "$HOME/%s/evidence.py" %s\n' % (self.bin_rel, encoded),
                           timeout=timeout)
    ...
    def run(self, cmd, flow="job", target="run", interval=5, expect=None,
        ...
        if workspace is not None and (
                not isinstance(workspace, str) or not workspace.startswith("/")
                or "\x00" in workspace or "\n" in workspace
                or "\r" in workspace or ".." in workspace.split("/")):
            return Result(UNKNOWN, self.host, reason="unsafe workspace")
        if not cmd:
...
def _reader_launcher(subcommand, args, reader_rel=READER_REL):
...
def _bundle_installer(files, manifest_hash, bin_rel=BIN_REL):
